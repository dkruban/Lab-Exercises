...
def entropy_detailed(data, target_attr='play', indent=""):
    ...
    print(f"\n{indent}Entropy Formula: H(S) = -sum of( p(x) * log2(p(x)))")
    ...
    for value, count in value_counts.items():
        ...
        if probability > 0:
            ...
            ent -= term
            terms.append(f"({count}/{total}) * log₂({count}/{total})")
...
def information_gain_detailed(data, attribute, target_attr='play'):
    ...
    print(f"\nFormula: H(S, {attribute}) = sum of (|Sv|/|S|) * H(Sv)")
    print(f"\nCalculation:")
...
def find_root_node(file_path):
    ...
    ignore = list(input("Enter the non attribute fields: "))
    attributes = [key for key in data[0].keys() if key not in ignore]
    ...
    for value, count in target_counts.items():
        print(f"  {value}: {count} ({count}/{len(data)} = {count/len(data):.2%})")
